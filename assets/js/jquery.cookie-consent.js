...
(function ($) {
  ...
  $.fn.cookieConsent = function (options) {
    ...
    settings.isGoogleBot = !!navigator.userAgent.match(
      /Chrome-Lighthouse|Page Speed|Headless/i
    );
    // console.log(settings);
    var language = window.navigator.userLanguage || window.navigator.language;
    settings.storage =
      settings.storage === "local" && typeof Storage !== "undefined"
        ? "local"
        : settings.storage === "session" && typeof Storage !== "undefined"
        ? "session"
        : "cookie";
    var consentedDate =
      settings.storage === "local"
        ? parseInt(localStorage.getItem(settings.consentKey))
        : settings.storage === "session"
        ? parseInt(sessionStorage.getItem(settings.consentKey))
    ...
    var elm = this.length
      ...
      : $("<div>", {
          ...
          style:
            "background-color:white;color:#333;text-align:center;display:none;" +
            settings.style,
        })
          .append(
            $("<button>", {
              ...
              style:
                "background:#e96a1f;color:white;border:none;border-radius:0.2em;margin: 0 auto;margin-top: .5rem;padding:0.2em 0.5em 0.2em 0.5em;display: block;" +
                settings.consentStyle,
    ...
    if (settings.isGoogleBot) {
      ...
      settings.testing ||
      !consentedDate ||
      consentedDate + 86400000 * settings.consentTime < new Date().getTime()
    ) {
      $(elm).show();
    } else {
        ...
        .click(function () {
          if (settings.storage === "local") {
            ...
            $.cookie(settings.consentKey, new Date().getTime(), {
              expires: new Date(
                new Date().getTime() + 86400000 * settings.consentTime
              ),
              path: "/",
            });
          }
